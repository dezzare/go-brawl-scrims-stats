...
func loadPlayers() {
	...
	players, err := handler.GetPlayersFromFile()
	if err != nil {
		fmt.Println(err)
		return
	}
	for _, v := range players.Player {
		...
		p, err := handler.GetPlayerByTag(v.Tag)
		v.Follow = true
		if err != nil && v.Tag != "" {
...
func setPlayersBrawlerStat(pbs *[]PlayerBrawlerStat, players *[]entity.Player) error {
	...
	if err := handler.GetBattleresultByPlayerIDs(playersID, &battleResults); err != nil {
		fmt.Println(err)
	}

	brawlers := handler.GetAllBrawlers()
